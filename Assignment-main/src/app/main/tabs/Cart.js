...
const Cart = ({ route }) => {
    ...
    return (
      <View style={{ flexDirection: 'row', marginTop: 16, width: '100%', height: 154, backgroundColor: '#262B33', borderRadius: 23, padding: 10 }}>
        ...
        <View style={{ marginLeft: 10 }}>
          ...
          <View style={{justifyContent:'flex-end', alignItems:'center', width:'100%', marginTop:5}}>
            <TouchableOpacity style={{ width: 20, height: 15, borderRadius: 15, backgroundColor: 'red', alignItems:'center',justifyContent:'center' }}>
              <Text style={{fontSize:8, fontWeight:'bold', color:'#fff'}}>X</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
  ...
  const payMent = async () => {
    ...
    if (totalPrice == 0) {
      return;
      Alert.alert('Khong the thanh toan');
    } else {
